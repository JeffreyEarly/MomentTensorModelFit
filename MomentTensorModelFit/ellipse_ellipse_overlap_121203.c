 ...
 *
 *  Reference: Hughes and Chraibi (2011-2012), Calculating Ellipse Overlap Areas
 *
 *  Dependencies: math.h  for calls to trig and absolute value functions  
 *                program_constants.h  error message codes and constants
 *
 *  Inputs:   1. double PHI_1  CCW rotation angle of first ellipse, radians
 *            2. double A1     semi-axis length in x-direction first ellipse
 *            3. double B1     semi-axis length in y-direction first ellipse 
 *            4. double H1     horizontal offset of center first ellipse
 *            5. double K1     vertical offset of center first ellipse
 *            6. double PHI_2  CCW rotation angle of second ellipse, radians
 *            7. double A2     semi-axis length in x-direction second ellipse
 *            8. double B2     semi-axis length in y-direction second ellipse
 *            9. double H2     horizontal offset of center second ellipse 
 *           10. double K2     vertical offset of center second ellipse
 *
 *  Outputs:  1. int *rtnCode  returns diagnostic information integer code
 *                                 integer codes in program_constants.h
 *
 *  Return:   The calculated value of the overlap area
 *            -1 is returned in case of an error with the calculation
 *             0 is returned if the ellipses are disjoint
...
//-- for the two ellipses
	...
	for (i = 0; i < nychk; i++)
	{
		...
		if (fabs(ellipse2tr(x1, ychk[i], AA, BB, CC, DD, EE, FF)) < EPS)
		{
			...
			yint = (double *)realloc(yint, nintpts*sizeof(double));
			if(ychk == NULL)
			{
